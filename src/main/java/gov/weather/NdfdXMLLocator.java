 ...
 * longitude pair and the product type. The Unit will default
 *       to U.S. Standard (english) unless Metric is chosen by client.
 * The client also needs to provide the start and end time (Local)
 *       of the period that it wants data for (if shorter than the 7
 * days is wanted).  For the time-series product, the client needs to
 * provide an array of boolean values corresponding to which NDFD values
 * are desired.
 ...
 *       pair is composed of a latitude and longitude delimited by a
 * comma.
 ...
 * Rico.
 *       For the LatLonListSquare function, the input is the same as
 * the NDFDgen function except the latitude and longitude values
 *       are relaced by a zip code for the 50 United States and Puerto
 * Rico.
 ...
 *       threshold.
 *       For the GmlLatLonList function, the service requires a list
 * of latitude and longitude pairs, the time (UTC) the user
 *       wants data for, the GML feature type and the array of boolean
 * values corresponding to which NDFD values are desired.
 *       For the GmlTimeSeries function, the service requires a list
 * of latitude and longitude pairs, the start and end time (UTC) the
 * user
